...
function ForumPage() {
  ...
  const categories = [
    ...
    { id: 'actualite', name: 'Actualités', color: 'amber-700' },
    { id: 'autre', name: 'Autre', color: 'gray-600' }
  ];
  ...
  useEffect(() => {
    ...
    const fetchTopics = async () => {
      try {
        ...
        if (!active) return;
        if (data.unavailable) throw new Error("Le forum est temporairement indisponible. Réessaie dans un instant.");
        setTopics(data.topics);
        setPagination(data.pagination);
      } catch (error) {
        if (active) setError(error.name === 'AbortError' ? "La connexion prend trop de temps. Réessaie." : error.message);
      } finally {
  ...
  }, [pagination.page, pagination.limit, category, submittedSearch, retry]);
  ...
  return (
    <div className="page py-10 md:py-14">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        <div>
          <h1 className="page-heading mb-2 relative inline-block">
            Le forum de la classe
          </h1>
        ...
        <button
          ...
          className="mt-4 md:mt-0 px-4 py-2 bg-biogy-700 hover:bg-biogy-800 text-white rounded-lg shadow-soft hover:shadow-soft  transition-all duration-300 flex items-center"
        >
      ...
      <div className="bg-white rounded-lg shadow-soft p-5 mb-6 sketch-container" style={{animationDelay: '0.1s'}}>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
          ...
          <div className="flex flex-wrap gap-2 mb-4 md:mb-0">
            {categories.map((cat, index) => (
              <button
                ...
                onClick={() => { setCategory(cat.id); setPagination((value) => ({ ...value, page: 1 })); }}
                className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-300 transform  ${
                  category === cat.id
                    ? `bg-${cat.color} text-white shadow-soft`
          ...
          <form onSubmit={handleSearch} className="w-full md:w-auto">
            <div className="relative group">
              <input
                type="text"
                aria-label="Rechercher un sujet" placeholder="Rechercher un sujet..."
                value={search}
              ...
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-400 group-hover:text-biogy-700 transition-colors duration-300" viewBox="0 0 20 20" fill="currentColor">
              ...
              <button
                ...
                className="absolute inset-y-0 right-0 pr-3 flex items-center text-biogy-700 hover:text-lab-blue transition-colors duration-300"
              >
      ...
      {/* Liste des sujets */}
      <div className="bg-white rounded-lg shadow-soft overflow-hidden sketch-container" style={{animationDelay: '0.2s'}}>
        {loading ? (
          <div className="p-12 text-center">
            <div className="inline-block animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-lab-purple mb-4">
              <div className="h-10 w-10 rounded-full bg-biogy-700/10"></div>
            </div>
          ...
          <div className="p-12 text-center">
            ...
            <button
              ...
              className="px-4 py-2 bg-biogy-700 hover:bg-biogy-800 text-white rounded-lg shadow-soft hover:shadow-soft  transition-all duration-300"
            >
              Réessayer
            </button>
          </div>
          ...
          <div className="p-12 text-center">
            <div className="mb-6 relative">
              <div className="absolute inset-0 bg-biogy-700/5 rounded-full "></div>
              <svg xmlns="http://www.w3.org/2000/svg" className="h-20 w-20 mx-auto text-biogy-700/70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button
              onClick={handleNewTopic}
              className="px-6 py-3 bg-biogy-700 hover:bg-biogy-800 text-white rounded-lg shadow-soft hover:shadow-soft  transition-all duration-300 font-medium"
            >
              Créer un sujet
            </button>
          </div>
          ...
          <>
            ...
            <div className="hidden md:grid grid-cols-12 gap-4 p-4 bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200 font-medium text-gray-700">
              <div className="col-span-6 flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-2 text-biogy-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="divide-y divide-gray-200">
              {topics.map((topic, index) => (
                <div
                  key={topic._id}
                  className={`p-5 hover:bg-gray-50 transition-all duration-300 ${topic.isSticky ? 'bg-biogy-700/5' : ''}`}
                  ...
                  <div className="md:grid md:grid-cols-12 md:gap-4 flex flex-col">
                    ...
                    <div className="col-span-6">
                      <Link to={`/forum/${topic._id}`} className="group block">
                        <h3 className="text-lg font-semibold text-gray-800 group-hover:text-biogy-700 transition-all duration-300 flex items-center">
                          {topic.isSticky && (
                            <span className="inline-flex mr-2 text-biogy-700 bg-biogy-700/10 p-1 rounded-full">
                        ...
                        <button
                          ...
                          className={`px-4 py-2 text-sm font-medium transition-all duration-300 ${
                            pagination.page === page + 1
                              ? 'bg-biogy-700 hover:bg-biogy-800 text-white shadow-soft'
                              : 'text-gray-700 bg-gray-100 hover:bg-gray-200 hover:text-biogy-700'
                          }`}
                        >
                          {page + 1}
                        </button>
