...
function ProfilePage() {
  ...
  // Charger le profil (version simplifiée qui fonctionne toujours)
  useEffect(() => {
    const loadProfile = () => {
      if (!userInfo) {
      ...
      try {
        setLoading(true);
        console.log('Création du profil simulé...');

        // Toujours utiliser un profil simulé pour garantir le fonctionnement
        const simulatedProfile = createSimulatedProfile(userInfo);
        setProfile(simulatedProfile);

        // Message informatif
        setError('Affichage d\'un profil simulé. Les modifications seront sauvegardées localement.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const createSimulatedProfile = (userInfo) => {
    ...
    if (storedProfile) {
      try {
        // Utiliser le profil stocké
        const profileData = JSON.parse(storedProfile);
        console.log('Profil récupéré du localStorage:', profileData);
        return profileData;
      } catch (error) {
        console.error('Erreur lors de la lecture du profil stocké:', error);
        // Continuer avec la création d'un nouveau profil
      }
    }
    ...
    // Sauvegarder le nouveau profil dans le localStorage
    localStorage.setItem(PROFILE_STORAGE_KEY, JSON.stringify(newProfile));
    console.log('Nouveau profil créé et sauvegardé:', newProfile);
