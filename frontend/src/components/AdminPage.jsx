...
import AdminLabSubmissionsPanel from './AdminLabSubmissionsPanel';
...
const UserManagement = ({ userInfo, onUnauthorized }) => {
  ...
  const resetPassword = async (user) => {
    if (!window.confirm(`Réinitialiser le mot de passe de ${user.username} ? Ses sessions actuelles seront fermées.`)) return;
    setResetting(user._id); setError(''); setResetResult(null);
    try {
      const data = await apiRequest(`/admin/users/${user._id}/reset-password`, { method: 'POST', headers: { Authorization: `Bearer ${userInfo.token}` } });
      setResetResult({ username: data.user.username, password: data.temporaryPassword });
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 mb-8">
      ...
      {resetResult && <div role="status" className="notice mb-5"><p className="font-semibold">Mot de passe temporaire · {resetResult.username}</p><p className="mt-2 font-mono text-lg select-all">{resetResult.password}</p><p className="mt-2">Communique ce mot de passe directement à l’élève. Il devra le changer à sa prochaine connexion.</p><button type="button" className="btn-secondary mt-3" onClick={() => setResetResult(null)}>Masquer le mot de passe</button></div>}
      {/* Barre de recherche */}
      <div className="mb-6">
        <input
          type="text"
          aria-label="Rechercher un utilisateur" placeholder="Rechercher un utilisateur..."
          value={searchTerm}
                    ...
                    {editingUser === user._id ? (
                      <>
                        {newUsername && (
                          <button
                            ...
                            className="text-biogy-600 hover:text-biogy-900 mr-2"
                          >
                            Enregistrer nom
                          </button>
                        )}
                      ...
                      <>
                        ...
                        <button
                          ...
                          disabled={Boolean(resetting) || user._id === userInfo._id}
                          className="min-h-11 text-biogy-700 hover:underline"
                        >
                          {resetting === user._id ? 'Réinitialisation…' : 'Réinitialiser le mot de passe'}
                        </button>
                        <button
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {loadingAuth ? (
        ...
        <AdminLabSubmissionsPanel userInfo={userInfo} onUnauthorized={handleUnauthorized} />
      )}
    </div>
  );
}
