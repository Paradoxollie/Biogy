...
function ScienceWatchPage() {
  ...
  const fetchArticles = useCallback(async (colorKey, { hardRefresh = false } = {}) => {
    ...
    try {
      ...
      if (requestRef.current === controller) setError(err.message);
    } finally {
  ...
  const renderCategoryRibbon = () => (
    <div className="sticky top-16 z-30 bg-white/95 backdrop-blur-md border-b border-gray-100 py-4 mb-10 overflow-x-auto hide-scrollbar shadow-sm">
      <div className="page flex items-center space-x-2 md:space-x-8">
        <button
  ...
  return (
    <div className="min-h-screen bg-gray-50/30">
      {/* Editorial Header */}
      <header className="page pb-8 pt-10 md:pt-14"><p className="section-eyebrow">Veille scientifique</p><h1 className="mt-3 font-display text-3xl md:text-display-lg">Le vivant fait l’actualité.</h1><p className="mt-4 max-w-2xl leading-relaxed text-ink-600">Explore les biotechnologies à travers des articles de presse et de recherche. Ouvre la source pour lire l’article complet et vérifier sa date.</p></header>
