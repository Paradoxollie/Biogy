...
import { safeRedirect, isDocumentPath } from '../utils/navigation';
...
function ChangePasswordPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (currentPassword === newPassword) {
      setError('Choisis un nouveau mot de passe différent du mot de passe temporaire.');
      return;
    }
    ...
    try {
      ...
      const target = safeRedirect(location.state?.from || '/');
      if (isDocumentPath(target)) window.location.assign(target);
    ...
    <div className="container mx-auto max-w-xl px-4 py-10">
      <div className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
        <div className="px-6 py-5 bg-gradient-to-r from-amber-50 via-white to-rose-50 border-b border-gray-100">
          ...
          <h1 className="mt-2 page-heading">Choisir un nouveau mot de passe</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                required
              />
              <p className="mt-1 text-sm text-gray-500">Utilise au moins 8 caractères.</p>
            </div>
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              <button
                type="submit"
                disabled={submitting}
                className="rounded-lg bg-biogy-700 hover:bg-biogy-800 px-5 py-3 font-semibold text-white transition hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-60"
              >
