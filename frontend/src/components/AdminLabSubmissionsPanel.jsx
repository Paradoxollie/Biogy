...
function AdminLabSubmissionsPanel({ userInfo, onUnauthorized }) {
  ...
  const fetchSubmissionDetail = useCallback(async (submissionId) => {
    ...
    try {
      ...
      if (request === detailRequest.current) setError(fetchError.message || 'Erreur lors du chargement de la copie.');
    } finally {
  ...
  const handleSaveReview = async () => {
    if (!selectedSubmission || savingReview) {
      return;
    }
    ...
    try {
      ...
      setSelectedSubmission((current) => current?._id === selectedSubmission._id ? {
        ...current,
        review: data.submission.review,
      } : current);

      setSavedNotice('Correction enregistrée.');
  ...
  return (
    <section className="space-y-6">
      ...
      {savedNotice && <p role="status" className="rounded-card bg-biogy-50 p-4 text-biogy-800">{savedNotice}</p>}
      {error ? (
