...
function ProjectsGallery() {
  ...
  const fetchProjects = useCallback(async () => {
    try {
      setLoading(true);
      setError('');
      const response = await fetch(`${apiUrl}/posts`, { signal: AbortSignal.timeout(15000) });
      ...
      if (response.headers.get('X-Biogy-Database-Unavailable') === '1') throw new Error('La galerie est temporairement indisponible. Réessaie dans un instant.');
      setProjects(data);
    } catch (err) {
  ...
  const handleLike = async (projectId) => {
    ...
    try {
      ...
      setError(error.message || 'Cette action n’a pas pu être enregistrée.');
    } finally {
      setLikeLoading(null);
    }
  };
  ...
  return (
    <div className="page py-10 md:py-14">
      <header className="mb-8 flex flex-wrap items-end justify-between gap-5"><div className="max-w-xl"><p className="section-eyebrow">Les projets de la classe</p><h1 className="page-heading mt-3">La science se partage.</h1><p className="mt-4 leading-relaxed text-ink-600">Observations, expériences et réalisations des élèves. Chaque projet est validé par l’enseignante avant publication.</p></div><Link to="/partager-projet" className="btn-primary">Partager un projet</Link></header>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-gray-50 rounded-lg sketch-border relative overflow-hidden">
          <div aria-hidden="true" className="pointer-events-none absolute top-0 left-0 w-full h-full">
        ...
        <div className="space-y-12">
          {projects.map((project) => {
            ...
            return (
            <div key={project._id} className="bg-white rounded-lg overflow-hidden hover:shadow-soft transition-all duration-300 sketch-container relative">
              {/* Éléments décoratifs */}
              <div className="absolute top-1 right-1 w-2 h-2 rounded-full bg-lab-blue/20"></div>
              <div className="absolute top-2 right-3 w-1 h-1 rounded-full bg-biogy-700/30"></div>
              ...
              <div className="p-5 sketch-content">
                ...
                {project.caption && (
                  <div className="mb-4 p-3 bg-yellow-50/30 sketch-note relative">
                    ...
                    <span className="font-medium mr-2 text-biogy-700">{projectAuthorName}:</span>
                ...
                {commenting === project._id && (
                  <div className="mt-4 flex space-x-2 sketch-comment-form">
                    ...
                    <button
                      ...
                      className="bg-biogy-700 text-white px-4 py-2 rounded-lg disabled:opacity-50 hover:bg-biogy-700/90 flex items-center sketch-button"
                    >
                      {commentLoading ? (
      ...
      <div className="text-center mt-16">
        <Link to="/partager-projet" className="px-8 py-4 bg-biogy-700 hover:bg-biogy-800 text-white rounded-lg shadow-soft hover:shadow-soft transition-all duration-300 transform  sketch-cta">
